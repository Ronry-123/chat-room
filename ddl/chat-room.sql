...
create table `room`(
    ...
    `db_create_time` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '数据库插入时间',
    `db_modify_time` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '数据库更新时间',
    `room_id` varchar(60) unique NOT NULL COMMENT '房间id',
...
create table `room_member`(
    ...
    `chat_uid` bigint(20) unsigned NOT NULL COMMENT '用户uid',
    `admin` tinyint(1) unsigned default 0 COMMENT '是否管理员',
    `agreed` tinyint(1) unsigned default 0 COMMENT '是否同意加入',
    primary key (`id`)
)ENGINE=InnoDB  comment 'room member';
...
create table `room_message`(
    ...
    primary key (`id`)
)ENGINE=InnoDB  comment 'room message';